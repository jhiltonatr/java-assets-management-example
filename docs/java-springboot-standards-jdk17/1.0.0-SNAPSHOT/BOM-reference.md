...
| unique packages scanned | `17` |
| unique vulnerable packages | `1` |
| vulnerabilities found | `11` |

Per application:

- `sample-springboot-app-jdk17`: `17` packages scanned, `1` vulnerable, `11` findings

None of the library versions managed in this BOM were resolved by any scanned application,
so the scan results do not annotate the Managed Dependencies table (the `known CVEs` column
stays empty). Full details: each app's `target/dependency-check-report.html`.
